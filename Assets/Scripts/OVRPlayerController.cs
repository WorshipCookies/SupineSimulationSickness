...
public class OVRPlayerController : MonoBehaviour
{
    ...
    */
    ...
    public bool ThirdPersonPointOfView = false;
    public float y3pp;
    public float z3pp;
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableRotation)
		{
			...
			else
			{
                ...
                {
                    if (OVRInput.Get(OVRInput.Button.SecondaryThumbstickDown) || OVRInput.Get(OVRInput.Button.SecondaryThumbstickUp))
                    {
        ...
        /*
        if(IsSecondaryThumbstickRotating() && GMS.GetIsButtonPressed())
        {
            Debug.Log("change Wall");
            GMS.SetIsButtonPressed(false);
            //GMS.CoolDownButtonStart();
        }
        */
    }
	...
	public void UpdateTransform(OVRCameraRig rig)
	{
        ...
        if (Input.GetKeyDown(KeyCode.T))
        {
            ...
            Debug.Log("Initial y : " + initY);
            timerAngle = Time.time;
            testAngle = true;
        }

        if(Time.time - timerAngle >= 1 && testAngle)
        {
            float testY = transform.localEulerAngles.y;
            Debug.Log("Test y : " + testY);
            Debug.Log("Dif : " + (testY - initY));
            testAngle = false;
        }
    ...
    private bool IsSecondaryThumbstickRotating()
    {
        return (OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick) != new Vector2(0, 0));
    }
    public void FixPositionAfterTeleport()
    {
