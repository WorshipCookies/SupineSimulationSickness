...
{
    public Boolean lyingPosWithoutTransition = false;
    public Boolean lyingPosWithTransition = false;
	/// <summary>
	/// The left eye camera.
	/// </summary>
    ...
    private Quaternion eul;
    private float x;
    private Vector3 initialRotation;

    private bool inclineCameraWithTransition = false;
...
#region Unity Messages
	...
	protected virtual void Start()
	{
        ...
        Debug.Log("Before Condition " + lyingPosWithoutTransition);
		UpdateAnchors();
        if (lyingPosWithoutTransition)
        {
    ...
    */
	...
	protected virtual void UpdateAnchors()
	{
        ...
        if (lyingPosWithoutTransition)
        {
            ...
            rightHandAnchor.localPosition = new Vector3(0, 0, 0.55f);

            
        } else
        {

            rightHandAnchor.localPosition = new Vector3(0, -0.55f, 0);
    ...
    public void StartRotate()
    {
        ...
        {
            inclineCameraWithTransition = true;
        }
        if(inclineCameraWithTransition)
        {
            //Vector3 targetDir = transform.up;
            //transform.localRotation = eul;
            // transform.localRotation = Quaternion.Euler(euler);

            //transform.localRotation = Quaternion.Euler(new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y, transform.localEulerAngles.z));
            if (transform.eulerAngles.x < 89)
            //if(transform.GetChild(0).eulerAngles.x < 89)
            {
                x += Time.deltaTime * 20;
                ...
                //transform.GetChild(0).localRotation = Quaternion.Euler(x, 0, 0);

                //transform.GetChild(0).localRotation.y

                //eul += Quaternion.Euler(new Vector3(9000, 0, 0) * Time.deltaTime *wd 0.5f);
                //euler += (new Vector3(90, 0, 0) * Time.deltaTime * 0.5f);
                //Quaternion rot = Quaternion.AngleAxis(90, targetDir);
    ...
    private void InclineCamera()
    {
        //transform.localEulerAngles = new Vector3(90, 0, 0);
        transform.localRotation = Quaternion.Euler(new Vector3(90, 0, 0));
        Enemy.SetActive(true);

        Debug.Log("Incline Camera");
    }
