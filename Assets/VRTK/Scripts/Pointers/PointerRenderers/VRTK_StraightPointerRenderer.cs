...
namespace VRTK
{
    ...
    public class VRTK_StraightPointerRenderer : VRTK_BasePointerRenderer
    {
        ...
        protected override void ChangeMaterial(Color givenColor, Material cursorMat)
        {
            base.ChangeMaterial(givenColor, cursorMat);
            ChangeMaterialColor(actualTracer, givenColor, cursorMat);
        }
        ...
        protected virtual void CheckRayMiss(bool rayHit, RaycastHit pointerCollidedWith)
        {
            if (!rayHit || (destinationHit.collider && destinationHit.collider != pointerCollidedWith.collider))
            {
                ...
                ChangeColor(invalidCollisionColor, customCursor, invalidMaterial);
                //Debug.Log("invalid" + invalidMaterial.name);
                //customCursor.transform.GetChild(0).gameObject.GetComponent<Renderer>().material = invalidMaterial;
        ...
        protected virtual void CheckRayHit(bool rayHit, RaycastHit pointerCollidedWith)
        {
            if (rayHit)
            {
                ...
                ChangeColor(validCollisionColor, customCursor, validMaterial);
                 //Debug.Log("valid" + validMaterial.name);
                //customCursor.transform.GetChild(0).gameObject.GetComponent<Renderer>().material = validMaterial;
...
#pragma warning restore 0618
        ...
        protected virtual void SetPointerAppearance(float tracerLength)
        {
            if (actualContainer)
            {
                ...
                actualCursor.transform.forward = new Vector3(0,0,0);
